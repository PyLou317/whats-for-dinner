  ...
  try {
    ...
    return data.meals.map(meal => {
      ...
      return {
        ...
        instructions: meal.strInstructions || 'No detailed instructions provided.',
      };
    });
  } catch (err) {
...
export async function fetchHouseholdMatches(householdId) {
  ...
  try {
    ...
    swipes.forEach((s) => {
      if (s.recipe_id) {
        ...
        if (s.recipes) {
          recipeMap[s.recipe_id] = s.recipes;
        }
      }
    });
