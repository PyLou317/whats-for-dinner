...
import OnboardingSwipeFlow from '../onboarding/OnboardingSwipeFlow.jsx';
import OnboardingDeckSection from '../onboarding/OnboardingDeckSection.jsx';
...
export default function SwipeDeck({
  ...
  const onOnboardingStateChange = (next) => setOnboardingState(next);
  ...
  useEffect(() => {
    fetchTodaySwipesAndRecipes({
      ...
      setPartnerSwipes,
      setLoading,
      onMatchesLoaded: onboardingOnly ? undefined : onMatchesLoaded,
      onOnboardingStateChange: (state) => {
  ...
  const handleSwipe = async (recipe, decision) => {
    if (!recipe) return;
    ...
    // IMPORTANT: use local-day todayStr from state (no UTC recompute here)
    const swipeDate = todayStr;

    if (isConfigured && user?.id && profile?.household_id && recipe?.id) {
      ...
      // Source-of-truth match check (reliable even if partnerSwipes state is stale)
      if (decision === 'yes') {
        const dbMatches = await fetchHouseholdMatches(
          profile.household_id,
          swipeDate,
        );
        const isNowMatched = dbMatches.some((m) => m.id === recipe.id);
        if (isNowMatched) {
  ...
  const handleUndoLastSwipe = async () => {
  ...
  const handleResetAllSwipes = async () => {
    const confirmed = window.confirm(
      "Reset all of today's swipes? This cannot be undone.",
    );
    if (!confirmed) return;
    ...
    // clear DB rows for this user + today
    if (isConfigured && profile?.household_id && user?.id) {
      try {
        ...
        if (error) {
          console.error('Failed to reset all swipes:', error);
          return;
        }
      } catch (err) {
        console.error('Failed to reset all swipes:', err);
        return;
      }
    }
  ...
  const onboardingKeptCount = useMemo(() => {
    const keptDecisions = new Set(['yes', 'right', 'like', true]);
    return Object.values(userSwipes || {}).filter((decision) =>
      keptDecisions.has(decision),
    ).length;
  }, [userSwipes]);
  ...
  if (onboardingOnly) {
    return (
      <OnboardingSwipeFlow
        currentIndex={swipedRecipeIds?.size || 0}
        total={onboardingState.total}
        showPrompt={!onboardingState.complete && availableCards.length > 0}
      >
        <OnboardingDeckSection
          loading={loading}
          availableCards={availableCards}
  ...
  return (
    ...
    >
      <div className="flex-1 flex flex-col justify-between p-4 max-w-md mx-auto w-full relative min-h-[82vh] pb-32">
        ...
        <div className="flex-1 flex items-center justify-center relative my-2 min-h-115">
          {loading ? (
            <LoadingState text="Loading today's meal stack..." />
          ) : availableCards.length > 0 ? (
            <div className="relative w-full h-115 flex items-center justify-center">
              <AnimatePresence>
            ...
            <EmptyDeckState
              onNavigateToCookbook={onNavigateToCookbook}
              handleResetSwipes={handleResetAllSwipes}
            />
          )}
        </div>
        ...
        {availableCards.length > 0 && (
          <div className="flex items-center justify-center gap-6 py-3 z-10">
            ...
            {/* Reset / Undo */}
            <ResetUndoBtn onReset={handleUndoLastSwipe} />
